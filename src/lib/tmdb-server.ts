...
export async function getFilmDetails(id: number): Promise<FilmDetails | null> {
...
export async function getFilmCredits(id: number): Promise<{cast: CastMember[], crew: CrewMember[]} | null> {
...
export async function getSimilarMovies(id: number): Promise<Film[] | null> {
    const data = await fetchFromTMDB<PaginatedResponse<any>>(`movie/${id}/similar`);
