    ...
    <div className="space-y-8">
      ...
      {journal.length > 0 ? (
        <div className="space-y-6">
          {journal.map((entry) => (
            <Card key={entry.film.id} className="bg-secondary border-0 md:flex overflow-hidden">
                <div className="md:w-48 flex-shrink-0 relative aspect-[2/3] md:aspect-auto">
                   <Link href={`/film/${entry.film.id}`} className="block h-full w-full">
                    <Image
                    ...
                    <CardHeader>
                        <CardTitle>
                          <Link href={`/film/${entry.film.id}`} className="hover:text-primary transition-colors">
                            <span className="font-headline text-2xl">{entry.film.title}</span>
                            <span className="text-muted-foreground font-normal text-lg ml-2">({entry.film.year})</span>
                          </Link>
                        </CardTitle>
